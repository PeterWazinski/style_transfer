...
@pytest.fixture()
def editor_dialog(qtbot) -> StyleEditorDialog:
    dlg = StyleEditorDialog()
    qtbot.addWidget(dlg)
    return dlg


@pytest.fixture()
def training_dialog(qtbot) -> TrainingProgressDialog:
    """Dialog with no real trainer — worker will not start."""
    dlg = TrainingProgressDialog(trainer=None)
    qtbot.addWidget(dlg)
    return dlg
...
class MockTrainer:
    """Helper that forwards progress directly to a dialog (no background thread)."""

    def __init__(self, dialog: TrainingProgressDialog) -> None:
        self._dialog = dialog

    def emit_progress(self, batch: int, total: int, eta_seconds: int) -> None:
        self._dialog.update_progress(batch, total, eta_seconds)

    def train(self, *, progress_callback=None, **kwargs) -> Path:  # noqa: ANN001
        return Path("mock_model.pth")


@pytest.fixture()
def mock_trainer(training_dialog: TrainingProgressDialog) -> MockTrainer:
    return MockTrainer(training_dialog)
